...
public class RemoveItemsnSlaughter implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("removeItems")) {
            ...
            switch (num) {
                case 0:
                    Resources.privateMsg(sender, "No items have been removed");
                    break;
                case 1:
                    Resources.privateMsg(sender, num + " item have been removed");
                    break;
                default:
                    Resources.privateMsg(sender, num + " items have been removed");
                    break;
            }
            ...
            for (LivingEntity ent : p) {
                if (ent.getLocation().distance(origin) <= radius && (!(ent instanceof Player) && (LetItRain.version >= 170 && !(ent instanceof ArmorStand)))) {
                    ent.setHealth(0);
                    num++;
                }
            }
            switch (num) {
                case 0:
                    Resources.privateMsg(sender, "No creature has been slaughtered");
                    break;
                case 1:
                    Resources.privateMsg(sender, num + " creature has been slaughtered");
                    break;
                default:
                    Resources.privateMsg(sender, num + " creatures have been slaughtered");
                    break;
            }
